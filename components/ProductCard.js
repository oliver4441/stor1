function ProductCard({ listing }) {
  return (
    <a href={`listing.html?id=${listing.id}`} className="block group">
      <div className="bg-zinc-100 dark:bg-zinc-900 rounded-2xl overflow-hidden aspect-square mb-3 relative border border-zinc-200/50 dark:border-zinc-800/50 transition-all group-hover:shadow-xl group-hover:border-primary/30">
      ...
      <div className="px-1">
        <div className="flex justify-between items-start gap-2 mb-0.5">
          <h3 className="font-bold text-zinc-900 dark:text-white truncate group-hover:text-primary transition-colors">{listing.title}</h3>
        </div>
        ...
        <p className="font-extrabold text-primary text-lg">{formatKES(listing.price)}</p>
      </div>
    </a>
  );
}