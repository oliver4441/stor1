    ...
    <div className="mt-8 pt-6 border-t border-zinc-200 dark:border-zinc-800">
      ...
      {reviews.length > 3 && (
        <button
          ...
          className="mt-3 flex items-center gap-1 text-sm font-bold text-[#ff385c] hover:underline mx-auto"
        >
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-6 p-5 bg-zinc-50 dark:bg-zinc-800/50 rounded-xl">
      ...
      <textarea
        ...
        className="w-full px-4 py-3 rounded-xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 text-sm resize-none h-24 focus:outline-none focus:border-[#ff385c]"
      />
      {error && <p className="text-red-500 text-xs mt-1">{error}</p>}
      <button
        type="submit"
        disabled={submitting || rating === 0}
        className="mt-3 px-6 py-2.5 bg-[#ff385c] text-white rounded-xl font-bold text-sm disabled:opacity-50 hover:bg-[#e03150] transition-all"
      >
