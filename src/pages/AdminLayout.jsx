...
import {
  ...
  LogOut, Shield, Menu, X, ChevronRight, Tag, Bell, Link as LinkIcon, MessageSquare, Gift, Zap, Store
} from 'lucide-react';
...
export default function AdminLayout() {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 flex">
      ...
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-zinc-900 border-r border-zinc-800 transform transition-transform duration-200 ease-in-out lg:translate-x-0 lg:static lg:z-auto ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex flex-col h-full">
          ...
          <div className="flex items-center justify-between h-16 px-5 border-b border-zinc-800">
            <Link to="/admin/dashboard" className="flex items-center gap-2.5">
              ...
              <span className="text-lg font-black text-white">Omix Admin</span>
            </Link>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-16 bg-zinc-900 border-b border-zinc-800 flex items-center justify-between px-4 lg:px-6 sticky top-0 z-30">
          <div className="flex items-center gap-3">
            <button onClick={() => setSidebarOpen(true)} className="lg:hidden p-2 rounded-xl hover:bg-zinc-800 text-zinc-400">
