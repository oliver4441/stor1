...
function Login() {
  ...
  return (
    <div className="max-w-md mx-auto px-4 py-20 w-full" data-name="login-page">
      ...
      <div className="fusion-clay-card p-6 space-y-4">
        {needsVerification && (
          <div className="bg-amber-900/20 border border-amber-900/50 text-amber-700 dark:text-amber-400 p-4 rounded-xl mb-4 text-sm">
            ...
            <p className="text-xs text-[#4A5771]">{t('auth.emailNotVerifiedHelp') || 'Need help?'} <Link to="/signup" className="text-[var(--seasonal-primary,#0d9488)] font-bold hover:underline">{t('auth.createNewAccount') || 'Create new account'}</Link>.</p>
          </div>
        )}
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <div>
            ...
            <input required name="email" type="email" placeholder={t('auth.emailPlaceholder') || 'your@email.com'} className="w-full px-4 py-3.5 rounded-xl bg-[#28303F] border border-transparent focus:border-[var(--seasonal-primary,#0d9488)] focus:bg-white dark:focus:bg-[#242C3B] focus:outline-none text-white transition-all shadow-sm" />
          </div>
          <div>
            <label className="block text-sm font-bold mb-2 text-[#8E9BB5]">{t('auth.password') || 'Password'}</label>
            <input required name="password" type="password" placeholder={t('auth.passwordPlaceholder') || '••••••••'} className="w-full px-4 py-3.5 rounded-xl bg-[#28303F] border border-transparent focus:border-[var(--seasonal-primary,#0d9488)] focus:bg-white dark:focus:bg-[#242C3B] focus:outline-none text-white transition-all shadow-sm" />
            <div className="flex justify-end mt-1.5">
              <button
                ...
                className="text-xs text-[#4A5771] hover:text-[var(--seasonal-primary,#0d9488)] transition-colors font-medium"
              >
                Forgot Password?
              </button>
            </div>
          </div>
          <button type="submit" disabled={loading} className="w-full bg-[var(--seasonal-primary,#0d9488)] text-white font-black py-4 rounded-2xl hover:bg-[var(--seasonal-secondary,#14b8a6)] transition-all disabled:opacity-50 shadow-lg shadow-[var(--seasonal-primary,#0d9488)]/20">
        ...
        <p className="text-center text-[#4A5771] text-sm">
          {t('auth.noAccount') || "Don't have an account?"} <Link to={`/signup${window.location.search}`} className="text-[var(--seasonal-primary,#0d9488)] font-bold hover:underline">{t('auth.signUp') || 'Sign Up'}</Link>
        </p>
      </div>
    </div>
  );
