...
const TIER_COLORS = {
  ...
  silver: { bg: 'bg-zinc-400/20', border: 'border-zinc-400/30', text: 'text-zinc-300', glow: 'shadow-zinc-400/20' },
  gold: { bg: 'bg-yellow-500/20', border: 'border-yellow-500/30', text: 'text-yellow-400', glow: 'shadow-yellow-500/20' },
  platinum: { bg: 'bg-blue-400/20', border: 'border-blue-400/30', text: 'text-blue-300', glow: 'shadow-blue-400/20' },
};
...
function AchievementCard({ ach }) {
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <span className="text-sm font-bold text-zinc-100">{ach.name}</span>
            ...
            <div className="flex items-center gap-2 mt-1.5">
              ...
              <span className="text-[11px] text-emerald-400 font-medium">Earned</span>
              {ach.earned_at && (
  ...
  return (
    <div className="rounded-xl p-4 border border-zinc-800/40 bg-zinc-900/30 opacity-60 hover:opacity-80 transition-opacity">
      <div className="flex items-start gap-3">
        <div className="w-10 h-10 rounded-lg bg-zinc-800/50 flex items-center justify-center text-lg text-zinc-600">
          <AchievementIcon icon={ach.icon} />
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className="text-sm font-bold text-zinc-500">{ach.name}</span>
            <span className="text-[10px] px-1.5 py-0.5 rounded font-medium bg-zinc-800/60 text-zinc-500 border border-zinc-700/30">
              {ach.tier.toUpperCase()}
            </span>
          </div>
          <p className="text-xs text-zinc-600 mt-0.5">{ach.description}</p>
          {/* Progress bar */}
          <div className="mt-2">
      ...
      <div className="bg-zinc-900/50 border-b border-zinc-800/50">
        <div className="max-w-4xl mx-auto px-4 py-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h1 className="text-xl font-bold text-zinc-100">Achievements</h1>
              <p className="text-sm text-zinc-400 mt-0.5">Earn badges and rewards as you grow</p>
            </div>
          </div>

          {/* Stats bar */}
          <div className="grid grid-cols-3 gap-3 mb-4">
            <div className="bg-zinc-900/70 rounded-xl p-3 text-center border border-zinc-800/30">
              <div className="text-2xl font-bold text-zinc-100">{totalCount}</div>
              <div className="text-[11px] text-zinc-500 uppercase tracking-wider mt-0.5">Total</div>
            </div>
            <div className="bg-zinc-900/70 rounded-xl p-3 text-center border border-zinc-800/30">
              <div className="text-2xl font-bold text-emerald-400">{earnedCount}</div>
              <div className="text-[11px] text-zinc-500 uppercase tracking-wider mt-0.5">Earned</div>
            </div>
            <div className="bg-zinc-900/70 rounded-xl p-3 text-center border border-zinc-800/30">
              <div className="text-2xl font-bold text-zinc-500">{totalCount - earnedCount}</div>
              <div className="text-[11px] text-zinc-500 uppercase tracking-wider mt-0.5">Locked</div>
            </div>
          </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
        {loading ? (
          <div className="space-y-3">
            {[1,2,3,4,5,6].map(i => (
              <div key={i} className="animate-pulse bg-zinc-900/50 rounded-xl h-20" />
            ))}
          </div>
        ) : sorted.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-zinc-500">No achievements found in this category</p>
          </div>
        ) : (
