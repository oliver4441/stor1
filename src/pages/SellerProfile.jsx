...
function Skeleton({ className = '' }) {
  return (
    <div className={`animate-pulse bg-[#28303F]/60 rounded ${className}`} />
  );
}

function TrustBadge({ icon: Icon, label, value, subtext, color = 'text-[#38B8EA]' }) {
  return (
    <div className="flex items-center gap-3 bg-[#28303F]/40 border border-zinc-700/50 rounded-xl p-4 transition-all duration-300 hover:border-zinc-600 hover:bg-[#28303F]/60">
      <div className={`p-2.5 rounded-lg bg-[#28303F] border border-zinc-700/60 ${color}`}>
        <Icon className="w-5 h-5" />
      </div>
      <div className="min-w-0">
        <p className="text-xs text-[#4A5771] uppercase tracking-wider">{label}</p>
        <p className="text-lg font-bold text-white truncate">{value}</p>
        {subtext && <p className="text-xs text-[#4A5771]">{subtext}</p>}
      </div>
    </div>
  );
}
    ...
    return (
      <div className="min-h-screen bg-zinc-950 text-white flex items-center justify-center">
        <div className="text-center max-w-md px-6">
          <Store className="w-16 h-16 text-[#4A5771] mx-auto mb-4" />
          <h2 className="text-xl font-semibold mb-2">Store Profile Unavailable</h2>
          <p className="text-[#4A5771] mb-6">{error}</p>
          <Link
            to="/"
            className="inline-flex items-center gap-2 px-5 py-2.5 bg-[#28303F] hover:bg-zinc-700 rounded-lg transition-colors"
          >
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white">
      ...
      <div className="max-w-5xl mx-auto px-4 -mt-16 relative z-10">
        <div className="flex flex-col md:flex-row md:items-end gap-4 mb-8">
          {/* Logo */}
          <div className="w-28 h-28 shrink-0 rounded-2xl bg-[#28303F] border-2 border-zinc-700 overflow-hidden flex items-center justify-center shadow-xl">
            {logo_url ? (
              <img
                src={logo_url}
                alt={store_name}
              ...
              <Store className="w-12 h-12 text-[#4A5771]" />
            )}
          </div>
          ...
          <div className="flex-1 pt-4 md:pt-0">
            ...
            {tagline && (
              <p className="text-[#8E9BB5] mt-1 text-sm md:text-base">{tagline}</p>
            )}
            {description && (
        ...
        <div className="mb-10">
          <div className="flex items-center gap-2 mb-4">
            <Shield className="w-5 h-5 text-[#38B8EA]" />
        ...
        <div className="mb-10">
          ...
          <div className="bg-[#28303F]/30 border border-zinc-700/40 rounded-xl p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {phone && (
                <a
                  href={`tel:${phone}`}
                ...
                >
                  ...
                  <div className="min-w-0">
                    <p className="text-xs text-[#4A5771] uppercase tracking-wider">Phone</p>
                    <p className="text-sm text-[#8E9BB5] group-hover:text-white transition-colors truncate">{phone}</p>
                  </div>
                </a>
              )}
              ...
              {email && (
                <a
                  ...
                  className="flex items-center gap-3 p-3 rounded-lg hover:bg-[#28303F]/50 transition-colors group"
                >
                  <div className="p-2 rounded-lg bg-[#28303F] border border-zinc-700/60 text-[#38B8EA] group-hover:text-emerald-300 transition-colors">
                    <Mail className="w-4 h-4" />
                  </div>
                  <div className="min-w-0">
                    <p className="text-xs text-[#4A5771] uppercase tracking-wider">Email</p>
                    <p className="text-sm text-[#8E9BB5] group-hover:text-white transition-colors truncate">{email}</p>
                  </div>
                </a>
              )}
              ...
              {whatsapp && (
                ...
                >
                  ...
                  <div className="min-w-0">
                    <p className="text-xs text-[#4A5771] uppercase tracking-wider">WhatsApp</p>
                    <p className="text-sm text-[#8E9BB5] group-hover:text-white transition-colors truncate">{whatsapp}</p>
                  </div>
                </a>
              )}
              ...
              {address && (
                <div className="flex items-center gap-3 p-3 rounded-lg">
                  <div className="p-2 rounded-lg bg-[#28303F] border border-zinc-700/60 text-rose-400">
                    <MapPin className="w-4 h-4" />
                  </div>
                  <div className="min-w-0">
                    <p className="text-xs text-[#4A5771] uppercase tracking-wider">Address</p>
                    <p className="text-sm text-[#8E9BB5] truncate">{address}</p>
                  </div>
                </div>
              )}
            </div>
